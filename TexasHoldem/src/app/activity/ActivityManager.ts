...
class ActivityManager
{
     ...
     * 签到处理
    */
    public static signInHandler=new SignInHandler();
    /**
     * 活动列表
     */
            ...
            for (let i: number = 0; i < result.data.length; i++)
            {
                ...
                switch (ActivityManager.list[i].definition.type)
                {
                    ...
                    case ActivityType.signin:
                        let lastTime:number=JSON.parse(result.data[i]["Json"]).SignTime;
                        let signInDayNum:number=ActivityManager.list[i].step;
                        ActivityManager.signInHandler.initialize(lastTime,signInDayNum);               
                }
            }
        }
